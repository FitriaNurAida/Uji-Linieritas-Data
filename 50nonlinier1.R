...
result50non1 <- matrix(0,ncol = 11,nrow = 10000)
colnames(result50non1) <- c("regresssor^2", "regresssor^3", "regresssor^23",
...
for (i in 1:10000)
{
  ...
  result50non1[i,1] <- prr2$p.value
  result50non1[i,2] <- prr3$p.value
  result50non1[i,3] <- prr23$p.value
  result50non1[i,4] <- prf2$p.value
  result50non1[i,5] <- prf3$p.value
  result50non1[i,6] <- prf23$p.value
  result50non1[i,7] <- prp2$p.value
  result50non1[i,8] <- prp3$p.value
  result50non1[i,9] <- prp23$p.value
  result50non1[i,10] <- pw$p.value
  result50non1[i,11] <- pt$p.value
}

write.csv(result50non1, "hasil5010000non1.csv")
...
for (i in 1:11)
{
  x[i]=length(result50non1[,i][result50non1[,i]<0.05])
  persen_benar[i]=x[i]/10000*100
}

z50non1=data.frame(uji_statistik, persen_benar,row.names = NULL)
z50non1
hasi50nonlinier1=write.csv(z50non1,"hasil50nonlinier1.csv")