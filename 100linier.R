...
result <- matrix(0,ncol = 11,nrow = 10000)
colnames(result) <- c("regresssor^2", "regresssor^3", "regresssor^23",
...
for (i in 1:10000)
{
  ...
  result[i,1] <- prr2$p.value
  result[i,2] <- prr3$p.value
  result[i,3] <- prr23$p.value
  result[i,4] <- prf2$p.value
  result[i,5] <- prf3$p.value
  result[i,6] <- prf23$p.value
  result[i,7] <- prp2$p.value
  result[i,8] <- prp3$p.value
  result[i,9] <- prp23$p.value
  result[i,10] <- pw$p.value
  result[i,11] <- pt$p.value
}

write.csv(result, "hasil10000.csv")
write.xlsx(result, "hasil10000.xlsx")
...
for (i in 1:11)
{
  x[i]=length(result[,i][result[,i]>0.05])
  persen_benar[i]=x[i]/10000*100
}

z100=data.frame(uji_statistik, persen_benar,row.names = NULL)
z100
hasil100linier=write.csv(z100,"hasil100linier.csv")
