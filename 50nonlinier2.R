...
result50non2 <- matrix(0,ncol = 11,nrow = 10000)
colnames(result50non2) <- c("regresssor^2", "regresssor^3", "regresssor^23",
...
for (i in 1:10000)
{
  ...
  x <- rmvnorm(n=50, mean=c(1,1,1), sigma=sigma)
  e <- rnorm(50)
  x1 <- x[,1]
  x2 <- x[,2]
  x3 <- x[,3]
  ...
  result50non2[i,1] <- prr2$p.value
  result50non2[i,2] <- prr3$p.value
  result50non2[i,3] <- prr23$p.value
  result50non2[i,4] <- prf2$p.value
  result50non2[i,5] <- prf3$p.value
  result50non2[i,6] <- prf23$p.value
  result50non2[i,7] <- prp2$p.value
  result50non2[i,8] <- prp3$p.value
  result50non2[i,9] <- prp23$p.value
  result50non2[i,10] <- pw$p.value
  result50non2[i,11] <- pt$p.value
}

write.csv(result50non2, "hasil5010000non2.csv")
...
for (i in 1:11)
{
  x[i]=length(result50non2[,i][result50non2[,i]<0.05])
  persen_benar[i]=x[i]/10000*100
}

z50non2=data.frame(uji_statistik, persen_benar,row.names = NULL)
z50non2
hasi50nonlinier2=write.csv(z50non2,"hasil50nonlinier2.csv")