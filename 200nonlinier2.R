...
result200non2 <- matrix(0,ncol = 11,nrow = 10000)
colnames(result200non2) <- c("regresssor^2", "regresssor^3", "regresssor^23",
...
for (i in 1:10000)
{
  ...
  result200non2[i,1] <- prr2$p.value
  result200non2[i,2] <- prr3$p.value
  result200non2[i,3] <- prr23$p.value
  result200non2[i,4] <- prf2$p.value
  result200non2[i,5] <- prf3$p.value
  result200non2[i,6] <- prf23$p.value
  result200non2[i,7] <- prp2$p.value
  result200non2[i,8] <- prp3$p.value
  result200non2[i,9] <- prp23$p.value
  result200non2[i,10] <- pw$p.value
  result200non2[i,11] <- pt$p.value
}

write.csv(result200non2, "hasil20010000non2.csv")
...
for (i in 1:11)
{
  x[i]=length(result200non2[,i][result200non2[,i]<0.05])
  persen_benar[i]=x[i]/10000*100
}

z200non2=data.frame(uji_statistik, persen_benar,row.names = NULL)
z200non2
hasi200nonlinier2=write.csv(z200non2,"hasil200nonlinier2.csv")